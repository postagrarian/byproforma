...
def _run(slot: int) -> dict:
    ...
    try:
        ...
        if active:
            # Align all active holdings on common dates, then weight-sum
            ret_df = pd.DataFrame({tk: returns_dict[tk] for tk in active}).dropna()
            if len(ret_df) >= 24:
                w_arr = np.array([active[tk] for tk in ret_df.columns])
                w_arr /= w_arr.sum()
                port_ret = pd.Series(ret_df.values @ w_arr, index=ret_df.index)
                port_betas = svc_reg.estimate_latest_betas(port_ret, factor_df)
                if port_betas:
                    portfolio_r2 = round(port_betas["r2"], 4)
                    print(f"[pipeline] ETF R²={etf_r2}  Portfolio R²={portfolio_r2}")
    except Exception as exc:
        print(f"[pipeline] Portfolio R² failed: {exc}")
