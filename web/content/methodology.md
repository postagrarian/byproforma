...
|---|---|---|---|
| ETF holdings & weights | Financial Modeling Prep (FMP) `/stable/etf/holdings` | Daily (fresh each run) | Not cached |
...
| YTD / 1Y total returns | FMP `/stable/stock-price-change` | Daily | Not cached |
| Equity price history | FMP `/stable/historical-price-eod/dividend-adjusted` | Monthly (EOD) | Supabase; refreshes if >45 days stale |
| Individual stock sectors | FMP `/stable/profile` | Static | Supabase `ticker_sectors`; permanent until overridden |
| Fama-French 5 factors + Momentum | Ken French Data Library (direct ZIP download) | Monthly | Supabase `ff_factors`; refreshes if >45 days stale |
...
```
r_i,t - RF_t  =  α_i  +  β_Mkt · (Rm-RF)_t  +  β_SMB · SMB_t  +  β_HML · HML_t
               +  β_RMW · RMW_t  +  β_CMA · CMA_t  +  β_Mom · Mom_t  +  ε_i,t
```
...
- **Minimum observations**: 70% of the window (≥25 months). Securities with insufficient history are excluded from the universe.
...
- **No constant restriction**: An intercept (alpha) is included in every regression but not used in the optimization. Alpha represents unexplained return and is not a factor loading target.
- **Index alignment**: Returns and factors are both normalised to month-start timestamps before regression to avoid date-mismatch artifacts from month-end vs. month-start indexing.

### R² interpretation

The R² of a regression measures what fraction of that security's return variance is explained by the six factors. For a broad market ETF like IJH, R² is typically 0.90–0.96 — the index is almost entirely systematic. For individual mid-cap stocks, R² commonly ranges from 0.20–0.70, reflecting meaningful idiosyncratic risk.

The replication portfolio's R² is computed by constructing the weighted portfolio return series and regressing it on the factors. A well-constructed replication portfolio should approach (but not match) the ETF's R² because 20–25 stocks retain more idiosyncratic risk than 400.
...
subject to:
  ...
  0  ≤  w_i  ≤  0.15                                (long-only, 15% position cap)
  |Σ_{i∈sector s} w_i  -  target_s|  ≤  0.03       (sector within ±3% of ETF)
```

Where:
- **B** is the (6 × n) matrix of factor betas for the n universe stocks
- **w** is the weight vector to solve for
- **β_ETF** is the ETF's own factor loading vector (from its 36-month OLS)
- **target_s** is the ETF's actual sector weight for sector s

### Solver

The QP is solved using **scipy's SLSQP** (Sequential Least Squares Programming) method via `scipy.optimize.minimize`. SLSQP handles nonlinear constraints and bounds efficiently for problems of this size (typically 100–200 variables).
...
After the QP converges, a sector-aware trimming loop reduces the number of positions to ≤25 (replication) or ≤40 (Active Tilt):

- Iterate over active positions from smallest to largest weight.
- Zero out a position if and only if removing it keeps every sector weight within the ±3% tolerance.
...
A lower RMSE indicates a tighter factor replication. Typical values for a 20–25 stock replication of a 400-stock index range from 0.02–0.15, depending on the breadth and factor diversity of the universe.
...
The Active Tilt draws from a merged universe of all stocks across all configured replication portfolios. With five portfolios each contributing 20–25 stocks (after deduplication), the tilt universe typically contains 80–120 unique securities — substantially larger than any single portfolio's universe and providing better factor-space coverage.
...
**Factor stationarity.** OLS betas are estimated on a trailing 36-month window, implicitly assuming that a security's factor exposures are approximately stationary within that window. Structural breaks (e.g., a major acquisition, business model change, or index re-classification) will cause the estimated betas to be a lagged average rather than an instantaneous reading.

**Single-period optimization.** The optimizer minimizes factor distance at a single point in time using the most recent beta estimates. It does not account for beta uncertainty, estimation error, or the covariance of estimation errors across securities. A Bayesian shrinkage approach (e.g., James-Stein or Ledoit-Wolf) would produce more stable portfolios but is not currently implemented.

**Equal weight on all six factors.** The objective treats deviations in Mkt-RF and Mom identically. In practice, the Mkt-RF beta matters more for total portfolio risk than the Mom beta. A weighted objective (e.g., downweighting smaller factors) could improve practical tracking.

**Long-only constraint.** The optimizer does not allow short positions. This restricts the achievable factor combinations, particularly for portfolios attempting extreme factor tilts (e.g., very high momentum with very low market beta). If the target is outside the convex hull of available long-only portfolios, the optimizer will find the nearest feasible point.

**Sector weights from FMP.** ETF sector weights are sourced from FMP's `/stable/etf/sector-weightings` endpoint and use Morningstar's sector classification. Individual stock sectors are also classified by Morningstar (via FMP's profile endpoint). Minor discrepancies in classification (e.g., a stock that Morningstar puts in "Technology" but the ETF classifies as "Communication Services") can introduce small sector constraint errors.

**Monthly refresh cadence.** The pipeline is scheduled to run on the 1st of each month. Factor loadings estimated mid-month may be 2–4 weeks stale by the time the next portfolio is produced. For investors with shorter rebalancing horizons this lag is material; for monthly or quarterly rebalancers it is not.

**Data vendor dependency.** Price history and ETF holdings are sourced from Financial Modeling Prep. Factor data is sourced from Ken French's data library. Disruptions to either source (API outages, methodology changes, data revisions) will affect pipeline output without warning.
...
|---|---|
| ETF holdings & sector weights | Every pipeline run |
| Fund overview (name, expense ratio, returns) | Every pipeline run |
| Price history | If last cached date is >45 days old |
| Fama-French factors | If last cached date is >45 days old |
...
| Replication portfolio | Automatically on the 1st of each month (Vercel Cron) |
